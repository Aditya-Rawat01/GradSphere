    ...
    <Card>
      ...
      <CardContent>
        <ul className="space-y-4">
          {visibleItems.map((e) => (
            <li key={e.id} className="p-3 border rounded-lg flex flex-col gap-2 hover:bg-muted/30 transition">
              <div className="flex justify-between items-center">
                <p className="font-semibold">{e.title}</p>
                <Badge variant="outline" className={e.type === "Virtual" ? "text-blue-600" : "text-green-600"}>
                  {e.type}
                </Badge>
              </div>
